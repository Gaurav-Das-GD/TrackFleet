...
export default function AdminDashboard() {
  const { data: analytics, isLoading: loadingAnalytics } = useQuery({
    queryKey: ['analytics'],
    queryFn: async () =>
      (await fleetApi.getAnalytics()).data.data as Analytics,
  ...
  const { data: fleet } = useQuery({
    queryKey: ['fleet'],
    queryFn: async () =>
      (await fleetApi.getFleet()).data.data as Driver[],
  ...
  const { data: deliveries, isLoading: loadingDeliveries } = useQuery({
    ...
    queryFn: async () =>
      (await deliveryApi.getAll({ limit: 8 })).data.data as Delivery[],
  });
    ...
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">
          TrackFleet Control Center
        </h1>

        <p className="text-muted-foreground mt-1">
          Transportation analytics and operational monitoring
        </p>
      </div>
        ...
        {loadingAnalytics ? (
          ...
          <>
            ...
            <StatCard
              ...
              icon={Truck}
              change="Real-time tracking enabled"
              trend="neutral"
            />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <Card>
            <CardHeader>
              <CardTitle>Live Vehicle Monitoring</CardTitle>
            </CardHeader>
